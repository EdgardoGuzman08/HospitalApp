...
const EliminarMedicos = ({ navigation }) => {
    ...
    const EliminarMedico= async()=>{
        ...
        if (!validarId) {
            await Axios.delete(
                '/medicos/eliminar?id='+id
                
            ).then((data) => {
                const json = data.data;
                console.log(json);
                Alert.alert('Eliminar Medico', 'Se eliminó con exito');
            }).catch((er) => {
                console.log(er);
            ...
            Alert.alert('Eliminar Medico', 'Se deben ingresar los datos correctamente');
        }
    }
    return (
