...
import Recetas from '../componetes/Recetas';
...
const ListarReceta = ({ navigation }) => {
    ...
    const BuscarTipo = async () => {
        if (!validarFiltro) {
            var textoMensaje = '';
            setEspera(true);
            await Axios.get('/recetas/buscarnombre?tratamiento=' + Filtro + '%')
                .then(async (data) => {
    ...
    const BuscarTipoTodos = async () => {
        var textoMensaje='';
        setEspera(true);
        await Axios.get('/recetas/listar')
            .then(async (data) => {
                const json = data.data;
                if (json.errores.length == 0) {
                    setLista(json.data);
                        ...
                        <>
                            <View style={Estilos.contenedorControles}>
                                ...
                                { 
                                    validarFiltro ? (
                                        <>
                                            <Text style={Estilos.etiquetaError}>Dede escribir el tratamiento</Text>
                                        </>
                            ...
                            <View style={Estilos.contenedorControles}>
                                <FlatList
                                    data={lista}
                                    renderItem={({ item }) => <Recetas receta={item}></Recetas>}
