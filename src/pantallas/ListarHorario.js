...
import Horarios from '../componetes/Horarios';
...
const ListarHorario = ({ navigation }) => {
    ...
    const BuscarTipo = async () => {
        if (!validarFiltro) {
            var textoMensaje = '';
            setEspera(true);
            await Axios.get('/horarios/buscarhora?fechaAtencion=' + Filtro + '%')
                .then(async (data) => {
    ...
    const BuscarTipoTodos = async () => {
        var textoMensaje='';
        setEspera(true);
        await Axios.get('/horarios/listar')
            .then(async (data) => {
                const json = data.data;
                if (json.errores.length == 0) {
                    setLista(json.data);
                        ...
                        <>
                            <View style={Estilos.contenedorControles}>
                                ...
                                { 
                                    validarFiltro ? (
                                        <>
                                            <Text style={Estilos.etiquetaError}>Dede escribir la fecha de atencion</Text>
                                        </>
                            ...
                            <View style={Estilos.contenedorControles}>
                                <FlatList
                                    data={lista}
                                    renderItem={({ item }) => <Horarios horario={item}></Horarios>}
