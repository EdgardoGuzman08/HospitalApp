...
const EliminarHorario = ({ navigation }) => {
    ...
    const titulo='Eliminar Horario';

    const EliminarHorarios= async()=>{
        console.log(id)

        if (!validarId) {
            await Axios.delete(
                '/horarios/eliminar?id='+id
                
            ).then((data) => {
                const json = data.data;
                console.log(json);
                Alert.alert('Eliminar Horarios', 'Se eliminó con exito');
            }).catch((er) => {
                console.log(er);
            ...
            Alert.alert('Eliminar Horarios', 'Se deben ingresar los datos correctamente');
        }
    }
    return (
