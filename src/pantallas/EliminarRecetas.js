...
const EliminarReceta = ({ navigation }) => {
    ...
    const EliminarRecetas= async()=>{
        ...
        if (!validarId) {
            await Axios.delete(
                '/recetas/eliminar?id='+id
                
            ).then((data) => {
                const json = data.data;
                console.log(json);
                Alert.alert('Eliminar Recetas', 'Se Eliminó con exito');
            }).catch((er) => {
                console.log(er);
            ...
            Alert.alert('Eliminar Recetas', 'Se deben ingresar los datos correctamente');
        }
    }
    return (
