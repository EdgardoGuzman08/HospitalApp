import { StyleSheet, Text, View, Button, ImageBackground, FlatList, ScrollView, TextInput, Alert, Pressable } from 'react-native';
import React, { useState, useEffect, useContext } from 'react';
import { Ionicons } from '@expo/vector-icons';
import Expedientes from '../componetes/Expediente';
...
const ListarExpedientes = ({ navigation }) => {
    ...
    const BuscarTipo = async () => {
        if (!validarFiltro) {
            var textoMensaje = '';
            setEspera(true);
            await Axios.get('/expedientes/buscarnombre?diagnostico' + Filtro + '%')
                .then(async (data) => {
    ...
    const BuscarTipoTodos = async () => {
        var textoMensaje='';
        setEspera(true);
        await Axios.get('/expedientes/listar')
            .then(async (data) => {
                const json = data.data;
                if (json.errores.length == 0) {
                    setLista(json.data);
                        ...
                        <>
                            <View style={Estilos.contenedorControles}>
                                ...
                                { 
                                    validarFiltro ? (
                                        <>
                                            <Text style={Estilos.etiquetaError}>Dede escribir el diagnostico</Text>
                                        </>
                            ...
                            <View style={Estilos.contenedorControles}>
                                <FlatList
                                    data={lista}
                                    renderItem={({ item }) => <Expedientes expediente={item}></Expedientes>}
